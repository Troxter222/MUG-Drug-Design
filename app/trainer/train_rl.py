...
class RLHyperparams:
    ...
    BASE_MODEL = Config.CHECKPOINTS_DIR / "mug_base_epoch_10.pth" # Или best_model.pth
...
def train_reinforcement():
    ...
    # Initialize Agent
    agent = MolecularVAE(vocab_size, Config.EMBED_SIZE, Config.HIDDEN_SIZE, Config.LATENT_SIZE, Config.NUM_LAYERS).to(Config.DEVICE)
    agent.load_state_dict(state_dict)
    
    # Initialize Prior (Frozen)
    prior = MolecularVAE(vocab_size, Config.EMBED_SIZE, Config.HIDDEN_SIZE, Config.LATENT_SIZE, Config.NUM_LAYERS).to(Config.DEVICE)
    ...
    for epoch in range(RLHyperparams.EPOCHS):
        ...
        # --- GENERATION PHASE ---
        with torch.no_grad():
            indices = agent.sample(RLHyperparams.BATCH_SIZE, Config.DEVICE, vocab, max_len=150, temp=1.0)
        ...
        for item in batch_data:
            ...
            log_prob_agent = torch.log_softmax(out_agent, -1).gather(2, tgt.unsqueeze(2)).squeeze(2).sum()
            
            # Prior Prob
            with torch.no_grad():
                out_prior, _, _ = prior(inp)
                log_prob_prior = torch.log_softmax(out_prior, -1).gather(2, tgt.unsqueeze(2)).squeeze(2).sum()
            
            # Augmented Reward with KL Penalty
            aug_reward = reward + RLHyperparams.KL_COEF * (log_prob_prior - log_prob_agent).item()
        ...
        if (epoch+1) % 5 == 0:
            logger.info(f"Ep {epoch+1:03d} | Avg Reward: {avg_R:.2f} | Batch: {len(batch_data)}")
            
        # Checkpoint
        if avg_R > best_avg_reward:
            best_avg_reward = avg_R
            torch.save(agent.state_dict(), Config.CHECKPOINTS_DIR / "mug_rl_best.pth")
