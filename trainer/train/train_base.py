...
def loss_function(recon_x, x, mu, logvar, pad_index, kl_weight):
    B, L, V = recon_x.shape
    recon_x = recon_x.reshape(B * L, V)
    x = x.reshape(B * L)
    
    CE = torch.nn.CrossEntropyLoss(ignore_index=pad_index, reduction='sum')(recon_x, x)
    KLD = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
    
    return CE + (KLD * kl_weight), CE.item(), KLD.item()
...
def train_supervised():
    ...
    # 2. Data
    # Assuming train_selfies.csv is in processed dir
    ...
    for epoch in range(Config.EPOCHS):
        ...
        for batch in progress:
            ...
            loss, ce, kld = loss_function(output, targets, mu, logvar, pad_idx, kl_weight)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 5.0)
            optimizer.step()
            
            total_loss += loss.item()
            progress.set_postfix({'Loss': loss.item()/batch.size(0), 'KL': kld/batch.size(0)})
