"""
MUG - COVID-19 MPro Specialist Trainer
Optimized for: Systemic antiviral drugs (Lipinski's Rule of 5)
Target: Covid MPro
"""
...
class Config:
    ...
    # Save Paths
    SAVE_DIR = "checkpoints_specialists/covid"
    ...
    # Target Settings
    TARGET_NAME = "covid"
    BASELINE = 6.5  # Moderate baseline for small protease inhibitors
...
class SpecialistReward:
    ...
    def _calculate_physchem_score(self, mol):
        """
        Lipinski's Rule of 5 for general systemic drugs (Viral).
        """
        score = 0.0
        ...
        # 1. Molecular Weight (< 500 is ideal, < 600 acceptable)
        if mw < 500: score += 0.2
        elif mw < 600: score += 0.1
        
        # 2. LogP (Not too greasy, not too polar)
        if 0.5 < logp < 5.0: score += 0.2
    ...
    def _compute_single_molecule(self, smi):
        ...
        # MW Filter
        if props['mw'] < 150: return -0.5, stats # Too small
        if props['mw'] > 750: return -0.5, stats # Too big
        ...
        # Final Equation
        # We value QED and Docking balanced with Toxicity
        tox_penalty = 0.0   # уже учтено в dock_score
    ...
    def get_reward_batch(self, smiles_list):
        ...
        for i, smi in enumerate(smiles_list):
            if smi in self.cache:
                scores[i], stats_list[i] = self.cache[smi]
            else:
                indices.append(i)
...
def train():
    ...
    print(f"🦠 STARTING COVID SPECIALIST | Target: {Config.TARGET_NAME} | Baseline: {Config.BASELINE}")
    ...
    torch.save(agent.state_dict(), f"{Config.SAVE_DIR}/final_covid.pth")
    print("\nDone.")
